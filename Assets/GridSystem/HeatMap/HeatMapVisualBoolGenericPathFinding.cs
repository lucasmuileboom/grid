...
public class HeatMapVisualBoolGenericPathFinding : MonoBehaviour
{
    private Grid<PathFindingNode> grid;
    private Mesh mesh;
    private bool UpdateMesh = false;

    public void SetGrid(Grid<PathFindingNode> grid) 
    {
        this.grid = grid;
    ...
    private void GridValueChanged(object sender, Grid<PathFindingNode>.OnGridChangedEventArgs e) 
    {
        UpdateMesh = true;
    }
    ...
    private void UpdateHeatMapVisual() 
    {
        for(int x = 0; x < grid.GetWidth(); x++) 
        {
            for(int y = 0; y < grid.GetHeight(); y++) 
            {
                ...
                PathFindingNode heatMapGridBoolObject = grid.GetValue(x, y);
                float gridValueNormalized = heatMapGridBoolObject.walkable ? 1f : 0f;
